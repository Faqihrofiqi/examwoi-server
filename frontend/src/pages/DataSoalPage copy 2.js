...
const DataSoalPage = () => {
  ...
  const [activePackageStatus, setActivePackageStatus] = useState(null); // Status dari paket aktif
  ...
  const [questionFormMessage, setQuestionFormMessage] = useState(''); // Hanya satu set message state
  ...
  const resetQuestionForm = useCallback(() => {
    ...
    setQuestionFormMessage(''); // Reset message
    setQuestionFormMessageType(''); // Reset message type
  ...
  const resetPackageForm = useCallback(() => {
    ...
    setPackageFormMessage(''); // Reset message
    setPackageFormMessageType(''); // Reset message type
  ...
  const handleSaveQuestion = async (event) => {
    ...
    if (!activePackageId) { // <--- Validasi berdasarkan activePackageId
      setQuestionFormMessage('Silakan pilih atau buat paket ujian terlebih dahulu.');
      setQuestionFormMessageType('danger');
      return;
    }
    ...
    if (questionInputMode === 'raw') {
      if (!rawTextInput.trim()) {
        setQuestionFormMessage('Raw text soal tidak boleh kosong.');
        setQuestionFormMessageType('danger');
        return;
      }
      ...
      questionDataToSend = {
        examPackageId: activePackageId, // <--- Gunakan activePackageId
      ...
      if (!structuredQuestionForm.questionText.trim() || structuredQuestionForm.options.filter(opt => opt.trim() !== '').length < 2 || !structuredQuestionForm.correctOptionId.trim()) {
        setQuestionFormMessage('Semua field pertanyaan, opsi, dan jawaban benar harus diisi.');
        setQuestionFormMessageType('danger');
        return;
      }
      ...
      if (!structuredQuestionForm.correctOptionId || !correctOptionExists) {
        setQuestionFormMessage('Pilih jawaban benar yang valid.');
        setQuestionFormMessageType('danger');
        return;
      }
    ...
    try {
      ...
      if (isEditingQuestion && currentQuestion) {
        if (isBatchMode) {
          setQuestionFormMessage('Tidak bisa mengedit soal dalam mode batch. Silakan gunakan mode single.');
          setQuestionFormMessageType('danger');
          return;
        }
        response = await apiClient.put(`/questions/${currentQuestion.id}`, questionDataToSend);
        setQuestionFormMessage('Soal berhasil diperbarui!');
        setQuestionFormMessageType('success');
      } else {
        response = await apiClient.post('/questions', questionDataToSend);
        if (questionDataToSend.isBatch) {
          setQuestionFormMessage(`Berhasil menambahkan ${response.data.count} soal dalam batch!`);
          setQuestionFormMessageType('success');
        } else {
          setQuestionFormMessage('Soal berhasil ditambahkan sebagai DRAFT!');
          setQuestionFormMessageType('success');
        }
      }
      ...
      setQuestionFormMessage(`Gagal menyimpan soal: ${err.response?.data?.message || err.message}`);
      setQuestionFormMessageType('danger');
    }
  };
  ...
  const handleEditSoal = (soal) => {
    ...
    if (soal.content && soal.content.questionText && soal.content.options && Array.isArray(soal.content.options)) {
      ...
      setStructuredAudioFile(null);
      setParsedPreview(null);
    } else if (soal.rawText && soal.rawText.includes('#') && soal.rawText.includes('--') && soal.rawText.includes('@')) {
      setQuestionInputMode('raw');
  ...
  const questionTableHeaders = [
    ...
    'Benar',
    'Tipe',
    'Status Soal', // Ganti menjadi status soal, karena sekarang soal punya status sendiri
    'Aksi'
  ];
  ...
  return (
    <div className="data-soal-page">
      <h1 className="page-title">MENU MANAJEMEN SOAL & PAKET</h1>
      ...
      <div className="soal-content-layout">
        ...
        <section className="soal-table-section">
            ...
            <div className="card-body d-flex justify-content-between align-items-center">
              ...
              {activeFacultyId && (
                <button
                  ...
                  onClick={() => {
                    // Logic toggle untuk form Paket Ujian
                    // Jika form paket sudah terlihat dan BUKAN dalam mode editing, sembunyikan
                    if (showPackageForm && !isEditingPackage) { // <--- PERBAIKAN: Gunakan showPackageForm dan isEditingPackage
                      setShowPackageForm(false);
                      // Jangan reset form di sini jika tujuan hanya toggle visibility
                    } else { // Jika form paket tersembunyi atau dalam mode editing, tampilkan form paket baru
                      resetPackageForm(); // Reset form paket dan set ke mode tambah baru (isEditingPackage=false)
                      setShowPackageForm(true); // Buka form paket
                      setShowQuestionForm(false); // Pastikan form soal tertutup
                      setShowAddCategoryForm(false); // Pastikan form kategori tertutup
                    }
                  }}
                  disabled={!activeFacultyId}
                >
                  <i className="material-icons">add</i> {showPackageForm && !isEditingPackage ? 'Tutup Form Paket' : 'Tambah Paket Baru'} {/* <--- PERBAIKAN TEKS */}
                </button>
              )}
            </div>
          </header>
          ...
          {activeFacultyId && showPackageForm && (
            <div className="package-form-card card mb-4"> {/* <--- UBAH CLASS NAME DI SINI */}
              <div className="card-header card-header-primary">
                  ...
                  <tbody>
                    {examPackages.length > 0 ? (
                      examPackages.map((pkg, index) => (
                        <tr key={pkg.id}
                          className={activePackageId === pkg.id ? 'table-active-row' : ''}
                          onClick={() => handleSelectPackage(pkg.id, pkg.name, pkg.status)}>
                          <td>{index + 1}</td>
                          <td>{pkg.name}</td>
                          <td>{pkg.description || '-'}</td>
                          <td>{pkg.durationMinutes || '-'}</td>
                          <td>{pkg.totalQuestions || 0}</td>
                          <td>
                    ...
                    <div className="card-body">
                      ...
                      {questionInputMode === 'raw' && (
                        <form onSubmit={handleSaveQuestion}>
                          ...
                          <button type="submit" className="btn btn-primary btn-sm ml-2" disabled={!activePackageId}>
                                ...
                                <tr key={soal.id}>
                                  ...
                                  <td>
                                    <span className={`badge badge-${soal.status === 'PUBLISHED' ? 'success' : soal.status === 'DRAFT' ? 'warning' : 'danger'}`}>
                                      {soal.status} {/* Mengambil status dari soal itu sendiri */}
                                    </span>
                                  </td>
                                  <td className="action-cell">
